...
def bookmarks_scanner(self, list):
    ...
    scan = Gtk.Button(label=_("Perform a scan"))
    delete = Gtk.Button(label=_("Delete invalid bookmarks"))
    delete.set_sensitive(False)

    treeview = Gtk.TreeView()
    treestore = Gtk.TreeStore(str, str)

    for i, column_title in enumerate([_("URL"), _("Status")]):
    ...
    bt_grid.attach(scan, 1, 0, 1, 1)
    bt_grid.attach(delete, 2, 0, 1, 1)
    bt_grid.set_column_homogeneous(True)

    label = Gtk.Label(_("Warning: In some cases, scan can show false positives"))
    ...
    label.set_property("margin-top", 15)
    ...
    scan.connect("clicked", lambda x: init_scan(\
    self, window, label, delete, list, treestore, self.tlsbool))

def init_scan(self, window, label, delete, list, treestore, bool):

     on_list = []
     off_list = []
     on = _("online")
     off = _("offline or unknown")

     d = dialog().info(_("Scanning Bookmarks"),\
     "<span size='small'>{}...\n\n{}.</span>".format(\
     _("This operation may take a while"),\
     _("It all depends on your internet speed and amount of bookmarks to scan")))

     for i in list:

         if not validators.url(i[3]):
             b = off
             off_list.append(i[3])
         else:
             c = is_url_valid(i[3], bool)
             if c:
                 b = on
                 on_list.append(i[3])
             else:
         ...
         treestore.append(None, [i[2], b])
         
     label.set_text("{} {} : {} {} | {} {}".format(len(list),\
     _("scanned"), len(on_list), on, len(off_list), off))

     if off_list: delete.set_sensitive(True)
     delete.connect("clicked", lambda x:\
     del_offline_bookmarks(self, window, off_list))
     d.destroy()

def del_offline_bookmarks(self, window, list):

    s = _("bookmark will be deleted")
    p = _("bookmarks will be deleted")
    ...
    .format(_("Clicking on OK"), len(list), m))

    if decision:
        for i in list: self.on_erase_bookmarks(i)
        window.close()
